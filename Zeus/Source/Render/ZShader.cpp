...
ZTessellationShader::ZTessellationShader(const std::filesystem::path& vertexShaderPath, const std::filesystem::path& fragShaderPath, const std::filesystem::path& tscShaderPath, const std::filesystem::path& tesShaderPath)
{
	MakeShader(vertexShaderPath, fragShaderPath, tscShaderPath,tesShaderPath);
...
void ZTessellationShader::MakeShader(const std::filesystem::path& vertexShaderPath, const std::filesystem::path& fragShaderPath, const std::filesystem::path& tscShaderPath, const std::filesystem::path& tesShaderPath)
{
	Create(z_Vertex, vertexShaderPath.string().c_str(), GL_VERTEX_SHADER);
	Create(z_Fragment, fragShaderPath.string().c_str(), GL_FRAGMENT_SHADER);
	Create(z_TessellationControl, tscShaderPath.string().c_str(), GL_TESS_CONTROL_SHADER);
	Create(z_TesselationEvaluation, tesShaderPath.string().c_str(), GL_TESS_EVALUATION_SHADER);
	LinkShader();
}

void ZTessellationShader::Attach() const
{
	glUseProgram(z_Program);
}

void ZTessellationShader::SetVec3(std::string_view name, Vec3 value) const
{
	const auto uniform = z_UniformCache.find(name.data());
	const auto string = std::format("Failed to find uniform {}", name);
...
void ZTessellationShader::SetMat4(std::string_view name, const Mat4& value) const
{
	const auto uniform = z_UniformCache.find(name.data());
	const auto string = std::format("Failed to find uniform {}", name);
...
void ZTessellationShader::SetInt(std::string_view name, int value) const
{
	const auto uniform = z_UniformCache.find(name.data());
	const auto string = std::format("Failed to find uniform {}", name);
...
void ZTessellationShader::SetFloat(std::string_view name, float value) const
{
	const auto uniform = z_UniformCache.find(name.data());
	const auto string = std::format("Failed to find uniform {}", name);
...
void ZTessellationShader::Create(uint32_t& shader, const char* path, uint32_t type)
{
	std::ifstream file;
...
void ZTessellationShader::LinkShader()
{
	z_Program = glCreateProgram();
	glAttachShader(z_Program, z_Vertex);
	glAttachShader(z_Program, z_Fragment);
	glAttachShader(z_Program, z_TessellationControl);
	glAttachShader(z_Program, z_TesselationEvaluation);
	glLinkProgram(z_Program);
	...
	glDeleteShader(z_Vertex);
	glDeleteShader(z_Fragment);
	glDeleteShader(z_TessellationControl);
	glDeleteShader(z_TessellationControl);
}
